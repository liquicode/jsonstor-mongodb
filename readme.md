...
- ***Almost every criteria is handed to the MongoDB driver whole.*** Twenty eight of `jsongin`'s thirty two query operators are MongoDB's own, so the server decides the criteria, sorts it, limits it and mutates by it - which is why this adapter is the baseline rather than a participant.
