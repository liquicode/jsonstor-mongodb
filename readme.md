...
- ***The server evaluates the criteria.*** A criteria is sent to MongoDB as a query, and the results are not checked again, except when it uses one of jsongin's own operators - `$eqx`, `$nex`, `$exprx` or `$noop` - which are left out of the query and checked by `jsongin`.
- ***The identifier is always `_id`.*** A `PrimaryKey` setting is ignored.
- The database and the collection are created on the first call if they do not exist.
